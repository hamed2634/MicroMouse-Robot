...
void loop() {
  ...
  accAngleY = (atan(-1 * AccX / sqrt(pow(AccY, 2) + pow(AccZ, 2))) * 180 / PI) - 3; // AccErrorY ~(-1.58)
  // === Read gyroscope data === //
  ...
  GyroX = GyroX + 1.52; // GyroErrorX ~(-0.56)
  GyroY = GyroY - 0.83; // GyroErrorY ~(2)
  GyroZ = GyroZ + 1.32; // GyroErrorZ ~ (-0.8)
...
void calculate_IMU_error() {
  ...
  while (c < 20) 
  {
    ...
    // Sum all readings
    AccErrorX = AccErrorX + ((atan((AccY) / sqrt(AccX * AccX + AccZ * AccZ)) * 180.0 / PI));
    AccErrorY = AccErrorY + ((atan(-1 * (AccX) / sqrt(AccY * AccY + AccZ * AccZ)) * 180.0 / PI));
    c++;
  }
  //Divide the sum by 200 to get the error value
  AccErrorX = AccErrorX / 20;
  AccErrorY = AccErrorY / 20;
  c = 0;
